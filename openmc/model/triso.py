...
def pack_trisos(radius, fill, domain_shape='cylinder', domain_length=None,
    ...
    def random_point_cube():
        ...
        return [uniform(r_min[0], r_max[0]),
                uniform(r_min[0], r_max[0]),
                uniform(r_min[0], r_max[0])]
    ...
    def random_point_cylinder():
        ...
        r = uniform(0, r_max[0]**2)**.5
        t = uniform(0, 2*pi)
        return [r*cos(t), r*sin(t), uniform(r_min[2], r_max[2])]
    ...
    def random_point_sphere():
        ...
        r = (uniform(0, r_max[0]**3)**(1/3) / (x[0]**2 + x[1]**2 + x[2]**2)**.5)
    ...
    def create_rod_list():
        """Generate sorted list of rods (distances between particle centers). A
        rod between particles p and q is only included in the list if the
        ...
        # all particles (take the index of and distance to the second nearest
        # neighbor since the nearest neighbor is self)
        d, n = tree.query(particles, k=2)
        d = d[:,1]
        n = n[:,1]

        # Create array of shape [n_particles, 3] where each row is particle
        # index, index of nearest neighbor, and distance to nearest neighbor
        a = np.dstack(([i for i in range(len(n))], n, d))[0]

        # Create array of shape [n_particles, 3] where each row is the index of
        # a nearest neighbor (second column of 'a'), the index of the particle
        # it is the nearest neighbor of, and the distance between them
        b = a[a[:,1].argsort()]
        b[:,[0, 1]] = b[:,[1, 0]]

        # Find the intersection between p and q: a list of particles who are
        # each other's nearest neighbors and the distance between them
    ...
    def apply_boundary_conditions(i, j):
        ...
        for k in range(3):
            if particles[i][k] < r_min[k]:
                particles[i][k] = r_min[k]
            elif particles[i][k] > r_max[k]:
                particles[i][k] = r_max[k]
            if particles[j][k] < r_min[k]:
                particles[j][k] = r_min[k]
            elif particles[j][k] > r_max[k]:
                particles[j][k] = r_max[k]
    ...
    def random_sequential_pack():
        ...
        radius = (3/4 * initial_packing_fraction * domain_volume /
                  (pi * n_particles))**(1/3)
        diameter = 2*radius
        ...
        for i in range(n_particles):
            ...
            # Randomly choose position of particle center within the domain and
            # continue sampling new center coordinates as long as there are any
            # overlaps
            while True:
               p = random_point()
               idx = cell_index(p, cell_length)
            ...
            # Determine which lattice cells are within one diameter of particle's
            # center and add this particle to the list of particles in those cells
            for idx in cell_list(p, diameter, cell_length):
                mesh[idx].append(p)
    ...
    def close_random_pack():
        ...
        while True:
            ...
            # overlapping particles. A rod between particles p and q is only
            # placed on the list if q has no closer neighbors than p.
            create_rod_list()
            ...
            while True:
                ...
                # Get indices of the two closest particles and the distance
                # between their centers
                d, i, j = pop_rod()

                reduce_outer_diameter()

                # Move particles the two closest particles apart so they are
                # separated by one outer diameter
    ...
    # Check for valid container geometry
    ...
    # Check for container dimensions and alculate the volume of the container
    ...
    r_min, r_max = get_boundary_extremes()
    ...
    # Check that the packing fractions for the close random packing and random
    # sequential packing algorithms are valid.
    ...
    # starting point to produce a close random pack with the desired final
    # particle radius
    if initial_packing_fraction != packing_fraction:
        ...
        # Set outer diameter initially to an arbitrary value that yields a
        # packing fraction of 1
        initial_outer_diameter = 2*(domain_volume/(n_particles*4/3*pi))**(1/3)

        # Set inner and outer diameter of particles. These values will change
        # during packing
        outer_diameter = [initial_outer_diameter]
        inner_diameter = [0]

        # List of rods arranged in a heap where each element contains the
        # distance between the particle centers and the indices of the two
        # particles
        rods = []

        # Mapping of particle ids to rods. Each key in the dict is the id of a
        # particle that is in the rod list, and the value is the id of its
        # nearest neighbor and the rod that contains them. The dict is used to
        # find rods in the priority queue and to mark removed rods so rods can
        # be "removed" without breaking the heap structure invariant.
        rods_map = {}

        # Two way dictionary used to look up which particles are located within
        # one diameter of a given lattice cell and which lattice cells a given
        # particle center is within one diameter of. This is used to speed up
        # the nearest neighbor search.
        mesh = defaultdict(set)
        mesh_map = defaultdict(set)

        # Generate non-overlapping particles using close random packing
        # algorithm
        close_random_pack()

    trisos = []
    for i in range(n_particles):
        trisos.append(openmc.model.TRISO(radius, fill, particles[i] + offset))
